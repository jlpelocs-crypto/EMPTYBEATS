...
class GamingHubApp:
    def __init__(self, root):
        ...
        self._has_adb = bool(shutil.which(self._get_adb_executable()))
        self._has_scrcpy = bool(shutil.which(self._get_scarpy_executable()))
        if not self._has_adb:
            self._log("Warning: 'adb' not found in PATH — device features disabled until adb is available.")
        if not self._has_scrcpy:
            self._log("Warning: 'scrcpy' not found in PATH — mirroring features disabled until scrcpy is available.")

        # Start background monitor thread
        self.monitor_thread = threading.Thread(target=self._hardware_monitor_loop, daemon=True)
        self.monitor_thread.start()

        # cache for scrcpy feature detection
        self._scrcpy_no_audio_supported = None
    ...
    def _hardware_monitor_loop(self):
        ...
        # Use non-blocking cpu_percent and poll expensive commands less often
        loop_count = 0
        # initialize cpu_percent internal measurement
        try:
        ...
        while self.monitoring:
            try:
                ...
                # Poll zram less frequently (every ~10s) because zramctl is relatively expensive
                if loop_count % 10 == 0:
                    self._zram_summary = self._get_zram_status()

                # Poll adb devices every ~2s
                if loop_count % 2 == 0:
            ...
            loop_count = (loop_count + 1) % 1000000
            time.sleep(1.0)
    ...
    def _update_ui_loop(self):
        ...
        # Update UI at a reasonable interval to avoid churn
...
'''
    ...
    def _get_scarpy_executable(self):
        # Prefer system scrcpy if available, otherwise fall back to common snap location
        exe = "scrcpy.exe" if self.is_windows else "scrcpy"
        found = shutil.which(exe)
        if found:
            return found
        # fallback to snap path on Linux
        snap_path = "/snap/bin/scrcpy"
        if not self.is_windows and os.path.exists(snap_path):
            return snap_path
        return exe

    def _scrcpy_supports_no_audio(self) -> bool:
        """Detect if the installed scrcpy supports the '--no-audio' flag.

        Cache the result to avoid repeated help invocations.
        """
        if self._scrcpy_no_audio_supported is not None:
            return self._scrcpy_no_audio_supported
        exe = self._get_scarpy_executable()
        try:
            p = subprocess.run([exe, "--help"], capture_output=True, text=True, timeout=3)
            help_text = (p.stdout or "") + (p.stderr or "")
            self._scrcpy_no_audio_supported = "--no-audio" in help_text
        except Exception:
            self._scrcpy_no_audio_supported = False
        return self._scrcpy_no_audio_supported
    ...
    def _on_mirror(self):
        ...
        # Add rendering flags to prioritize smooth display
        args = [self._get_scarpy_executable(), "-m", resolution, "-b", bitrate, f"--max-fps={fps}", "--render-expired-frames"]
        if self._scrcpy_supports_no_audio():
    ...
    def _on_record(self):
        ...
        bitrate = self._get_selected_bitrate()
        fps = self._get_selected_fps()
        args = [self._get_scarpy_executable(), "-m", resolution, "-b", bitrate, f"--max-fps={fps}"]
        if self._scrcpy_supports_no_audio():
            args.append("--no-audio")
        args.extend(["--record", fullpath])
    ...
    def _start_process(self, cmd, title: str):
        ...
        try:
            if isinstance(cmd, str):
                ...
                args = cmd
            # For scrcpy processes, avoid piping stdout but keep stderr piped so we can capture error messages
            is_scrcpy = ("scrcpy" in title or title.startswith("scrcpy-"))
            if is_scrcpy:
            ...
            if self.is_windows:
                popen = subprocess.Popen(args, stdout=stdout_opt, stderr=stderr_opt, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
            else:
                popen = subprocess.Popen(args, stdout=stdout_opt, stderr=stderr_opt, preexec_fn=os.setsid)

            self.proc = ProcessWrapper(popen, title)

            # Only spawn reader threads when streams are pipes
            if stdout_opt is subprocess.PIPE:
                stdout_thread = threading.Thread(target=self._read_stream, args=(popen.stdout, title), daemon=True)
                stdout_thread.start()
            if stderr_opt is subprocess.PIPE:
    ...
    def _read_stream(self, stream, title: str):
        try:
            # Support both text and binary streams
