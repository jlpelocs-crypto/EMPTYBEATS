...
REM Ensure scrcpy is installed (try winget, then choco)
echo [4.5/5] Checking scrcpy installation...
powershell -nologo -noprofile -Command "if (Get-Command scrcpy -ErrorAction SilentlyContinue) { exit 0 } else { exit 1 }"
if %ERRORLEVEL% NEQ 0 (
    echo scrcpy not found. Attempting to install...
    where winget >nul 2>nul
    if ERRORLEVEL 0 (
        echo Installing scrcpy via winget...
        winget install --id Genymobile.scrcpy -e || echo winget install failed
    ) else (
        where choco >nul 2>nul
        if ERRORLEVEL 0 (
            echo Installing scrcpy via chocolatey...
            choco install -y scrcpy || echo choco install failed
        ) else (
            echo Could not find winget or chocolatey to auto-install scrcpy. Please install scrcpy manually.
        )
    )
) else (
    echo scrcpy found
